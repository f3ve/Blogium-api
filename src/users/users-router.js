      ...
      .then((hasUserWithUsername) => {
        ...
        return UsersService.hashPassword(password).then((hashPassword) => {
          const newUser = {
            ...
            bio,
            img:
              'https://firebasestorage.googleapis.com/v0/b/blogium-c1c36.appspot.com/o/default.png?alt=media&token=726bde4d-720e-44b0-9c36-d5ac25380046',
            date_created: 'now()',
          };
...
usersRouter
  ...
  .delete(requireAuth, (req, res, next) => {
    ...
    UsersService.deleteUser(app.get('db'), params.user_id)
      .then(() => {
        res.status(204).end();
      })
      .catch((err) => next(err));
...
usersRouter.route('/:user_id/posts').get((req, res, next) => {
  UsersService.getUserPosts(req.app.get('db'), req.params.user_id)
    .then((posts) => {
