...
const PostsService = {
  _getAllPosts(db) {
    return db
      ...
      .leftJoin('comments AS comm', 'p.id', 'comm.post_id')
  ...
  getPublished(db) {
    return PostsService._getAllPosts(db).where('p.published', true);
  },

  getUserPublishedPosts(db, userId) {
    return PostsService._getAllPosts(db).where('p.user_id', userId);
  },
  ...
  getById(db, id) {
    return PostsService._getAllPosts(db).where('p.id', id).first();
  },

  deletePost(db, id) {
    return db.from('posts').where('id', id).delete();
  },

  updatePost(db, id, newPostFields) {
    return db.from('posts').where('id', id).update(newPostFields);
  },
