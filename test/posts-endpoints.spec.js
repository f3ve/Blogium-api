...
describe('Posts Endpoints', () => {
  ...
  const { testUsers, testPosts, testComments } = helpers.makeBlogFixtures();
  ...
  describe(`GET /api/posts`, () => {
    ...
    context('Given posts', () => {
      beforeEach('insert data', () =>
        helpers.seedBlogTables(db, testUsers, testPosts, testComments)
      );
      ...
      it('responds with 200 and all of the published posts', () => {
        ...
        return supertest(app).get('/api/posts').expect(200, expectedPost);
      });
    });
    ...
    context('Given an xss attack', () => {
      ...
      beforeEach('insert malicious content', () => {
        return helpers.seedMaliciousData(db, testUser, maliciousPost);
      });
    ...
    context('happy path', () => {
      ...
      it('responds 201, serialized post', () => {
        ...
        const newPost = {
          title: 'New test post',
          content:
            'THis is the text content of the new test post, Lets hope this works....',
    ...
    context('given posts', () => {
      ...
      it('responds with 204 and removes the post from the database', () => {
        ...
        const expectedPosts = testPosts
          .filter((p) => p.id !== postId && p.published === true)
          .map((p) => helpers.makeExpectedPost(testUsers, p, testComments));
  ...
  describe('GET /api/posts/drafts', () => {
    ...
    context('given posts', () => {
      ...
      it('responds with 200 and all of the users drafts', () => {
        ...
        const usersDrafts = testPosts.filter(
          (p) => p.published === false && p.user_id === testUser.id
        );
        const expectedPosts = usersDrafts.map((p) =>
          helpers.makeExpectedPost(testUsers, p, testComments)
        );
